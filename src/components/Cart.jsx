...
const Cart = () => {
    ...
    const [newAddress, setNewAddress] = useState({ id: '', name: '', street: '', city: '', postalCode: '', phone: '' });
    const [formErrors, setFormErrors] = useState({});
    const [isPaymentModalOpen, setIsPaymentModalOpen] = useState(false);
    const [isSignupOpen, setIsSignupOpen] = useState(false);

    const totalAmount = itemsList.reduce((total, item) => total + item.price * item.quantity, 0);
    ...
    const decreaseQuantity = (itemId) => {
        ...
        const updatedTotalItemCount = itemsList.reduce(
            (total, item) => total + (item.id === itemId ? item.quantity - 1 : item.quantity),
            0
        );
    ...
    if (itemsList.length === 0) {
        return (
            <div className="flex justify-center items-center h-screen w-screen bg-white">
                <p className="text-gray-500 text-lg relative bottom-32">Your cart is empty</p>
            </div>
        );
    }
    return (
        <div className="flex justify-center items-start p-6 bg-white flex-wrap-reverse gap-3">
            <div className="w-full sm:w-2/3 lg:w-1/2 bg-white p-6 rounded-lg shadow-lg">
                {!authUser ? (
                    ...
                    <div>
                        ...
                        <h2 className="text-xl font-semibold mb-4 text-black">Choose a Delivery Address</h2>
                        {addresses.length > 0 ? (
                            <div>
                                {addresses.map((address) => (
                                    <div
                                        key={address.id}
                                        className={`mb-4 p-4 rounded-lg shadow-md cursor-pointer hover:bg-gray-100 transition ${selectedAddress?.id === address.id ? 'border-2 border-black' : ''}`}
                                        onClick={() => handleChooseAddress(address)}
                                    >
                                        <h3 className="font-medium text-lg text-black">{address.name}</h3>
                            ...
                            <p className="text-gray-500">No addresses available. Please add a new address.</p>
                        )}
                        {selectedAddress ? (
                            <button
            ...
            <div className="w-full sm:w-2/3 lg:w-1/3 bg-white p-6 rounded-lg shadow-lg">
                        ...
                        <div className="p-2 flex justify-between text-green-900">
                            <button className="bg-white text-green-900 rounded-xl border-2 w-24 h-10 relative">
                                <div className="p-2 flex justify-between text-green-900">
                                    ...
                                    <FaPlus onClick={() => addToCart(item)} className="pl-1 cursor-pointer hover:text-green-800" />
                                </div>
                            </button>
                        </div>
                    </div>
                ...
                <div className="border-t border-gray-300 mt-4 pt-4">
                    <p className="text-lg font-semibold text-black">Total Amount: ₹{totalAmount}</p>
                </div>
            </div>
